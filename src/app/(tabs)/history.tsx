...
export default function HistoryScreen() {
  ...
  return (
    <ScrollView className="flex-1 bg-light-bg dark:bg-dark-bg" showsVerticalScrollIndicator={false}>
      <View className="px-5 pt-12 pb-28">
          ...
          <View>
            {history.map((period, index) => (
              <Animated.View
                ...
                style={{ marginBottom: index === history.length - 1 ? 0 : 18 }}
              >
                <Pressable
                  className="bg-light-card dark:bg-dark-card border border-light-border dark:border-dark-border rounded-3xl p-6 active:border-primary-500/50 shadow-sm"
                  onPress={() => router.push(`/summary/${period.id}` as any)}
                >
                  <View className="flex-row justify-between items-start mb-4">
                    <Text className="text-light-text dark:text-white text-xl font-sans-bold">
                      {period.period_name}
                    </Text>
                    {period.is_locked ? (
                      <View className="bg-emerald-500/10 px-3 py-1.5 rounded-full border border-emerald-500/20">
                        <Text className="text-emerald-400 font-sans-bold text-xs uppercase tracking-wider">
                          Selesai
                        </Text>
                      </View>
                    ) : (
                      <View className="bg-primary-500/10 px-3 py-1.5 rounded-full border border-primary-500/20">
                        <Text className="text-primary-400 font-sans-bold text-xs uppercase tracking-wider">
                          Aktif
                        </Text>
                      </View>
                    )}
