...
export default function SettingsScreen() {
  ...
  const [activePicker, setActivePicker] = useState<'language' | 'currency' | 'theme' | null>(null);
  ...
  const toggleBiometric = async (value: boolean) => {
    if (value) {
      ...
      if (!hasHardware || !isEnrolled) {
        Alert.alert(
          'Tidak Didukung',
          'Perangkat Anda tidak memiliki fitur biometrik atau belum diatur.',
        );
        return;
      }
  ...
  return (
    <ScrollView
      className="flex-1 bg-light-bg dark:bg-dark-bg px-5 pt-20"
      showsVerticalScrollIndicator={false}
    >
      <View className="mb-10">
        <Text className="text-light-text dark:text-white text-5xl font-bold tracking-tight">
          {t('settings', language)}
        </Text>
      </View>
      ...
      <View className="mb-10">
        <Pressable
          className="bg-light-card dark:bg-dark-card rounded-3xl p-6 flex-row items-center active:opacity-70 shadow-sm"
      ...
      <View className="space-y-8 mb-10">
        ...
        <View className="mb-4">
          ...
          <View className="bg-light-card dark:bg-dark-card rounded-3xl overflow-hidden shadow-sm">
            <Pressable
              className="px-6 py-5 border-b border-light-border dark:border-dark-border flex-row justify-between items-center active:bg-light-border dark:active:bg-dark-border"
            ...
            >
              <View className="flex-row items-center">
                ...
                <Text className="text-light-text dark:text-white text-base font-medium">
            ...
            >
              <View className="flex-row items-center">
                ...
                <Text className="text-light-text dark:text-white text-base font-medium">
                  {t('payPeriod', language)}
                </Text>
              </View>
              <Ionicons name="chevron-forward" size={20} color="#475569" />
            </Pressable>
            <Pressable
              className="px-6 py-5 flex-row justify-between items-center active:bg-light-border dark:active:bg-dark-border"
        ...
        <View className="mb-4">
          ...
          <View className="bg-light-card dark:bg-dark-card rounded-3xl overflow-hidden shadow-sm">
            <View className="px-6 py-5 border-b border-light-border dark:border-dark-border flex-row justify-between items-center">
              <View className="flex-row items-center">
      ...
      <SelectionSheet<AppLanguage>
        ...
        options={[
          ...
          },
          {
            value: 'en',
            label: 'English',
            description: 'Use English labels',
            icon: 'globe-outline',
          },
        ]}
      ...
      />
      <SelectionSheet<AppCurrency>
        visible={activePicker === 'currency'}
        title="Pilih Mata Uang"
        value={currency}
        options={[
          { value: 'IDR', label: 'IDR — Rupiah Indonesia', icon: 'cash-outline' },
          { value: 'USD', label: 'USD — Dolar AS', icon: 'logo-usd' },
          { value: 'SGD', label: 'SGD — Dolar Singapura', icon: 'cash-outline' },
          { value: 'MYR', label: 'MYR — Ringgit Malaysia', icon: 'cash-outline' },
          { value: 'EUR', label: 'EUR — Euro', icon: 'cash-outline' },
        ]}
        onSelect={setCurrency}
        onClose={() => setActivePicker(null)}
      />
      <SelectionSheet<AppThemePreference>
        ...
        options={[
          ...
          },
          {
            value: 'dark',
            label: 'Mode Gelap',
            description: 'Nyaman digunakan malam hari',
            icon: 'moon-outline',
          },
          {
            value: 'light',
            label: 'Mode Terang',
            description: 'Tampilan cerah dan bersih',
            icon: 'sunny-outline',
          },
        ]}
        onSelect={setTheme}
